...
// Global Settings
// ===============
var appversion = "0.3.26";
var sipjsversion = "0.20.0";
var navUserAgent = window.navigator.userAgent;  // TODO: change to Navigator.userAgentData
var instanceID = String(Date.now());
var localDB = window.localStorage;
// var dtmf_counter = 0;
// Set the following to null to disable
var welcomeScreen = "<div class=\"UiWindowField\"><pre style=\"font-size: 12px\">";



var loadAlternateLang = (getDbItem("loadAlternateLang", "0") == "1"); // Enables searching and loading for the additional language packs other thAan /en.json
var availableLang = ["fr", "ja", "zh-hans", "zh", "ru", "tr", "nl", "es", "de", "pl", "pt-br"]; // Defines the language packs (.json) available in /lang/ folder

/**
 * Image Assets
 * Note: You can specify the assets to use below in array format
 */
var imagesDirectory = getDbItem("imagesDirectory", "");     // Directory For Image Assets eg: images/ 
var defaultAvatars = getDbItem("defaultAvatars", "avatars/default.0.webp,avatars/default.1.webp,avatars/default.2.webp,avatars/default.3.webp,avatars/default.4.webp,avatars/default.5.webp,avatars/default.6.webp,avatars/default.7.webp,avatars/default.8.webp"); 
var wallpaperLight = getDbItem("wallpaperLight", "wallpaper.light.webp");  // Wallpaper for Light Theme
var wallpaperDark = getDbItem("wallpaperDark", "wallpaper.dark.webp");     // Wallpaper for Dark Theme
...
var SipDomain = getDbItem("SipDomain", null);           // eg: raspberrypi.local
var SipUsername = getDbItem("SipUsername", null);       // eg: webrtc
var SipPassword = getDbItem("SipPassword", null);       // eg: webrtc

var SingleInstance = (getDbItem("SingleInstance", "1") == "1");      // Un-registers this account if the phone is opened in another tab/window
...
var TransportReconnectionAttempts = parseInt(getDbItem("TransportReconnectionAttempts", 999));   // The number of times to attempt to reconnect to a WebSocket when the connection drops.
var TransportReconnectionTimeout = parseInt(getDbItem("TransportReconnectionTimeout", 3));       // The time in seconds to wait between WebSocket reconnection attempts.

var SubscribeToYourself = (getDbItem("SubscribeToYourself", "0") == "1");              // Enable Subscribe to your own uri. (Useful to understand how other buddies see you.)
var VoiceMailSubscribe = (getDbItem("VoiceMailSubscribe", "1") == "1");                // Enable Subscribe to voicemail
var VoicemailDid = getDbItem("VoicemailDid", "");                                      // Number to dial for VoicemialMain()
var SubscribeVoicemailExpires = parseInt(getDbItem("SubscribeVoicemailExpires", 300)); // Voceimail Subscription expiry time (in seconds)
var ContactUserName = getDbItem("ContactUserName", "");                                // Optional name for contact header uri
var userAgentStr = getDbItem("UserAgentStr", "Browser Phone "+ appversion +" (SIPJS - "+ sipjsversion +") "+ navUserAgent);   // Set this to whatever you want.
var hostingPrefix = getDbItem("HostingPrefix", "");                                    // Use if hosting off root directory. eg: "/phone/" or "/static/"
var RegisterExpires = parseInt(getDbItem("RegisterExpires", 300));                     // Registration expiry time (in seconds)
var RegisterExtraHeaders = getDbItem("RegisterExtraHeaders", "{}");                    // Parsable Json string of headers to include in register process. eg: '{"foo":"bar"}'
var RegisterExtraContactParams = getDbItem("RegisterExtraContactParams", "{}");        // Parsable Json string of extra parameters add to the end (after >) of contact header during register. eg: '{"foo":"bar"}'
var RegisterContactParams = getDbItem("RegisterContactParams", "{}");                  // Parsable Json string of extra parameters added to contact URI during register. eg: '{"foo":"bar"}'
var WssInTransport = (getDbItem("WssInTransport", "1") == "1");                        // Set the transport parameter to wss when used in SIP URIs. (Required for Asterisk as it doesn't support Path)
...
var BundlePolicy = getDbItem("BundlePolicy", "balanced");                              // SDP Media Bundle: max-bundle | max-compat | balanced https://webrtcstandards.info/sdp-bundle/
var IceStunServerJson = getDbItem("IceStunServerJson", "");                            // Sets the JSON string for ice Server. Default: [{ "urls": "stun:stun.l.google.com:19302" }] Must be https://developer.mozilla.org/en-US/docs/Web/API/RTCConfiguration/iceServers
...
var SubscribeBuddyAccept = getDbItem("SubscribeBuddyAccept", "application/pidf+xml");  // Normally only application/dialog-info+xml and application/pidf+xml
var SubscribeBuddyEvent = getDbItem("SubscribeBuddyEvent", "presence");                // For application/pidf+xml use presence. For application/dialog-info+xml use dialog 
var SubscribeBuddyExpires = parseInt(getDbItem("SubscribeBuddyExpires", 300));         // Buddy Subscription expiry time (in seconds)
var ProfileDisplayPrefix = getDbItem("ProfileDisplayPrefix", "");                      // Can display an item from your vCard before your name. Options: Number1 | Number2
var ProfileDisplayPrefixSeparator = getDbItem("ProfileDisplayPrefixSeparator", "");    // Used with profileDisplayPrefix, adds a separating character (string). eg: - ~ * or even 💥
var InviteExtraHeaders = getDbItem("InviteExtraHeaders", "{}");                       // Extra SIP headers to be included in the initial INVITE message for each call. (Added to the extra headers in the DialByLine() parameters. e.g {"foo":"bar"})

var NoAnswerTimeout = parseInt(getDbItem("NoAnswerTimeout", 120));          // Time in seconds before automatic Busy Here sent
...
var StartVideoFullScreen = (getDbItem("StartVideoFullScreen", "1") == "1"); // Starts a video call in the full screen (browser screen, not desktop)
var SelectRingingLine = (getDbItem("SelectRingingLine", "1") == "1");       // Selects the ringing line if you are not on another call ()

var UiMaxWidth = parseInt(getDbItem("UiMaxWidth", 1240));                                   // Sets the max-width for the UI elements (don't set this less than 920. Set to very high number for full screen eg: 999999)
var UiThemeStyle = getDbItem("UiThemeStyle", "system");                                     // Sets the color theme for the UI dark | light | system (set by your systems dark/light settings)
var UiMessageLayout = getDbItem("UiMessageLayout", "middle");                               // Put the message Stream at the top or middle can be either: top | middle 
var UiCustomConfigMenu = (getDbItem("UiCustomConfigMenu", "0") == "1");                     // If set to true, will only call web_hook_on_config_menu
var UiCustomDialButton = (getDbItem("UiCustomDialButton", "0") == "1");                     // If set to true, will only call web_hook_dial_out
var UiCustomSortAndFilterButton = (getDbItem("UiCustomSortAndFilterButton", "0") == "1");   // If set to true, will only call web_hook_sort_and_filter
var UiCustomAddBuddy = (getDbItem("UiCustomAddBuddy", "0") == "1");                         // If set to true, will only call web_hook_on_add_buddy
var UiCustomEditBuddy = (getDbItem("UiCustomEditBuddy", "0") == "1");                       // If set to true, will only call web_hook_on_edit_buddy({})
var UiCustomMediaSettings = (getDbItem("UiCustomMediaSettings", "0") == "1");               // If set to true, will only call web_hook_on_edit_media
var UiCustomMessageAction = (getDbItem("UiCustomMessageAction", "0") == "1");               // If set to true, will only call web_hook_on_message_action
...
var EchoCancellation = (getDbItem("EchoCancellation", "1") == "1");      // Attempts to remove echo over the line.
var NoiseSuppression = (getDbItem("NoiseSuppression", "1") == "1");      // Attempts to clear the call quality of noise.
...
var MaxVideoBandwidth = parseInt(getDbItem("MaxVideoBandwidth", "2048")); // Specifies the maximum bandwidth (in Kb/s) for your outgoing video stream. e.g: 32 | 64 | 128 | 256 | 512 | 1024 | 2048 | -1 to disable
var videoAspectRatio = getDbItem("AspectRatio", "1.33");                  // Suggests an aspect ratio (1:1 = 1 | 4:3 = 0.75 | 16:9 = 0.5625) to your webcam.
...
var MaxDataStoreDays = parseInt(getDbItem("MaxDataStoreDays", 0));          // Defines the maximum amount of days worth of data (calls, recordings, messages, etc) to store locally. 0=Stores all data always. >0 Trims n days back worth of data at various events where. 
...
var RecordingVideoSize = getDbItem("RecordingVideoSize", "HD");             // The size/quality of the video track in the recordings (SD = 640x360 | HD = 1280x720 | FHD = 1920x1080)
var RecordingVideoFps = parseInt(getDbItem("RecordingVideoFps", 12));       // The Frame Per Second of the Video Track recording
var RecordingLayout = getDbItem("RecordingLayout", "them-pnp");             // The Layout of the Recording Video Track (side-by-side | them-pnp | us-only | them-only)
...
var MaxDidLength = parseInt(getDbItem("MaxDidLength", 16));          // Maximum length of any DID number including international dialled numbers.
...
var Language = getDbItem("Language", "auto");                        // Overrides the language selector or "automatic". Must be one of availableLang[]. If not defaults to en.

// Buddy Sort and Filter
var BuddySortBy = getDbItem("BuddySortBy", "activity");                      // Sorting for Buddy List display (type|extension|alphabetical|activity)
var SortByTypeOrder = getDbItem("SortByTypeOrder", "e|x|c");                 // If the Sorting is set to type then describe the order of the types.
var BuddyAutoDevareAtEnd = (getDbItem("BuddyAutoDevareAtEnd", "0") == "1");  // Always put the Auto Devare buddies at the bottom
var HideAutoDevareBuddies = (getDbItem("HideAutoDevareBuddies", "0") == "1");    // Option to not display Auto Devare Buddies (May be confusing if newly created buddies are set to auto devare.)
var BuddyShowExtenNum = (getDbItem("BuddyShowExtenNum", "0") == "1");        // Controls the Extension Number display

// Permission Settings
var EnabvarextMessaging = (getDbItem("EnabvarextMessaging", "1") == "1");               // Enables the Text Messaging
...
var DisableBuddies = (getDbItem("DisableBuddies", "0") == "1");                         // Removes the Add Someone menu item and icon from the profile area. Buddies will still be created automatically. Please also use MaxBuddies or MaxBuddyAge
var Enabvarransfer = (getDbItem("Enabvarransfer", "1") == "1");                         // Controls Transferring during a call
...
var IntercomPolicy = getDbItem("IntercomPolicy", "enabled");                            // disabled = feature is disabled | enabled = feature is always on
...
var EnableAlphanumericDial = (getDbItem("EnableAlphanumericDial", "0") == "1");         // Allows calling /[^\da-zA-Z\*\#\+\-\_\.\!\~\'\(\)]/g default is /[^\d\*\#\+]/g 
var EnableVideoCalling = (getDbItem("EnableVideoCalling", "1") == "1");                 // Enables Video during a call
var EnabvarextExpressions = (getDbItem("EnabvarextExpressions", "1") == "1");           // Enables Expressions (Emoji) glyphs when texting
var EnabvarextDictate = (getDbItem("EnabvarextDictate", "1") == "1");                   // Enables Dictate (speech-to-text) when texting
var EnableRingtone = (getDbItem("EnableRingtone", "1") == "1");                         // Enables a ring tone when an inbound call comes in.  (media/Ringtone_1.mp3)
var MaxBuddies = parseInt(getDbItem("MaxBuddies", 999));                                // Sets the Maximum number of buddies the system will accept. Older ones get devared. (Considered when(after) adding buddies)
var MaxBuddyAge = parseInt(getDbItem("MaxBuddyAge", 365));                              // Sets the Maximum age in days (by latest activity). Older ones get devared. (Considered when(after) adding buddies)
var AutoDevareDefault = (getDbItem("AutoDevareDefault", "1") == "1");                   // For automatically created buddies (inbound and outbound), should the buddy be set to AutoDevare.

var ChatEngine = getDbItem("ChatEngine", "SIMPLE");    // Select the chat engine XMPP | SIMPLE

// XMPP Settings
var XmppServer = getDbItem("XmppServer", "");                // FQDN of XMPP server HTTP service";
var XmppWebsocketPort = getDbItem("XmppWebsocketPort", "");  // OpenFire Default : 7443
var XmppWebsocketPath = getDbItem("XmppWebsocketPath", "");  // OpenFire Default : /ws
var XmppDomain = getDbItem("XmppDomain", "");                // The domain of the XMPP server
var profileUser = getDbItem("profileUser", null);            // Username for auth with XMPP Server eg: 100
// XMPP Tenanting
var XmppRealm = getDbItem("XmppRealm", "");                    // To create a tenant like partition in XMPP server all users and buddies will have this realm prepended to their details.
var XmppRealmSeparator = getDbItem("XmppRealmSeparator", "-"); // Separates the realm from the profileUser eg: abc123-100@XmppDomain
// TODO
var XmppChatGroupService = getDbItem("XmppChatGroupService", "conference");

// TODO
var EnableSendFiles = false;          // Enables sending of Images
var EnableSendImages = false;          // Enables sending of Images
var EnableAudioRecording = false;  // Enables the ability to record a voice message
var EnableVideoRecording = false;  // Enables the ability to record a video message
var EnableSms = false;             // Enables SMS sending to the server (requires onward services)
var EnableFax = false;             // Enables Fax sending to the server (requires onward services)
var EnableEmail = false;           // Enables Email sending to the server (requires onward services)
...
var audioBlobs = {}
var newLineNumber = 1;
var telNumericRegEx = /[^\d\*\#\+]/g
var telAlphanumericRegEx = /[^\da-zA-Z\*\#\+\-\_\.\!\~\'\(\)]/g

var settingsMicrophoneStream = null;
var settingsMicrophoneStreamTrack = null;
var settingsMicrophoneSoundMeter = null;

var settingsVideoStream = null;
var settingsVideoStreamTrack = null;

var CallRecordingsIndexDb = null;
var CallQosDataIndexDb = null;
...
$(document).ready(function () {
    ...
    if(options.BuddyAutoDevareAtEnd !== undefined) BuddyAutoDevareAtEnd = options.BuddyAutoDevareAtEnd;
    if(options.HideAutoDevareBuddies !== undefined) HideAutoDevareBuddies = options.HideAutoDevareBuddies;
    if(options.BuddyShowExtenNum !== undefined) BuddyShowExtenNum = options.BuddyShowExtenNum;
    if(options.EnabvarextMessaging !== undefined) EnabvarextMessaging = options.EnabvarextMessaging;
    ...
    if(options.Enabvarransfer !== undefined) Enabvarransfer = options.Enabvarransfer;
    ...
    if(options.EnabvarextExpressions !== undefined) EnabvarextExpressions = options.EnabvarextExpressions;
    if(options.EnabvarextDictate !== undefined) EnabvarextDictate = options.EnabvarextDictate;
...
function PrepareIndexDB(){
    ...
    // CallQosData
    // ===========
    var CallQosDataOpenRequest = window.indexedDB.open("CallQosData", 1);
    ...
    CallQosDataOpenRequest.onsuccess = function(event) {
        ...
        if(CallQosDataIndexDb.objectStoreNames.contains("CallQos") == false){
            ...
            // Drop the Database
            var DBDevareRequest = window.indexedDB.devareDatabase("CallQos");
            DBDevareRequest.onerror = function(event) {
                console.error("Error devaring database CallQos");
            }
            DBDevareRequest.onsuccess = function(event) {
                console.log("Database devared successfully");
    ...
    // Call Recordings
    // ===============
    var CallRecordingsOpenRequest = window.indexedDB.open("CallRecordings", 1);
    // If this is the first visit to this page, this would have now made an empty IndexDB
    ...
    CallRecordingsOpenRequest.onsuccess = function(event) {
        ...
        if(CallRecordingsIndexDb.objectStoreNames.contains("Recordings") == false){
            ...
            console.log("IndexDB is closed.");
            // Drop the Database
            var DBDevareRequest = window.indexedDB.devareDatabase("CallRecordings");
            DBDevareRequest.onerror = function(event) {
                console.error("Error devaring database CallRecordings");
            }
            DBDevareRequest.onsuccess = function(event) {
                console.log("Database devared successfully");
...
function AddSomeoneWindow(numberStr){
    ...
    html += "<div class=UiText>Auto Devare:</div>";
    html += "<div><input type=checkbox id=AddSomeone_AutoDevare><label for=AddSomeone_AutoDevare>"+ lang.yes +"</label></div>";
    html += "</div>";
        ...
        action: function(){
            ...
            if(type == "extension"){
                ...
                json.DataCollection.push(
                    {
                        ...
                        AutoDevare: $("#AddSomeone_AutoDevare").is(':checked')
                    }
                );
                buddyObj = new Buddy("extension", 
                                        ...
                                        $("#AddSomeone_AutoDevare").is(':checked'));
...
function EditBuddyWindow(buddy){
    ...
    html += "<div class=UiText>Auto Devare:</div>";
    html += "<div><input type=checkbox id=AddSomeone_AutoDevare "+ ((buddyJson.AutoDevare == true)? "checked" : "" ) +"><label for=AddSomeone_AutoDevare>"+ lang.yes +"</label></div>";

    // TODO, add option to devare data, etc, etc
    html += "<div class=UiText><button onclick=\"RemoveBuddy('"+ buddyObj.identity +"')\" class=\"UiDevareButton\"><i class=\"fa fa-trash\"></i> "+ lang.devare_buddy +"</button></div>";
    ...
    OpenWindow(html, lang.edit, 480, 640, false, true, lang.save, function(){
        ...
        buddyJson.AutoDevare = $("#AddSomeone_AutoDevare").is(':checked');
        buddyObj.AllowAutoDevare = buddyJson.AutoDevare;
        ...
        $("#ImageCanvas").croppie('result', varraints).then(function(base64) {
...
function InitUi(){
    ...
    // Search
    leftHTML += "<span id=divFindBuddy class=searchClean><INPUT id=txtFindBuddy type=text autocompvare=none style=\"width: calc(100% - 78px);\"></span>";
    ...
    CreateUserAgent(wssServer,SipDomain,WebSocketPort,SipUsername,SipPassword,ServerPath);
}
...
// Create User Agent
// =================
function CreateUserAgent(wssServer,SipDomain,WebSocketPort,SipUsername,SipPassword,ServerPath) {
    ...
    var options = {
        logConfiguration: false,            // If true, varructor logs the registerer configuration.
    ...
    userAgent.registrationCompvared = false;
    ...
    if(RegisterExtraHeaders && RegisterExtraHeaders != "" && RegisterExtraHeaders != "{}"){
        try{
            ...
            for (var [key, value] of Object.entries(registerExtraHeaders)) {
                if(value != ""){
    ...
    if(RegisterExtraContactParams && RegisterExtraContactParams != "" && RegisterExtraContactParams != "{}"){
        try{
            ...
            for (var [key, value] of Object.entries(registerExtraContactParams)) {
                if(value == ""){
...
function onTransportConnectError(error){
    ...
    // We set this flag here so that the re-register attempts are fully compvared.
    userAgent.isReRegister = false;
...
function Register() {
    ...
    $(".reg_signal").html("Registering");
    $(".reg_signal").addClass("bg-warning");
    if($(".reg_signal").hasClass("bg-success")){
    $(".reg_signal").removeClass("bg-success");
    }
    if($(".reg_signal").hasClass("bg-danger")){
        $(".reg_signal").removeClass("bg-danger");
    }
    if (userAgent == null) return;
...
function onRegistered(){
    ...
    $(".reg_signal").html("Registered");
    if($(".reg_signal").hasClass("bg-danger")){
        $(".reg_signal").removeClass("bg-danger");
    }
    if($(".reg_signal").hasClass("bg-warning")){
        $(".reg_signal").removeClass("bg-warning");
    }
    $(".reg_signal").addClass("bg-success");
    // $(".reg_signal").parent().append("\n<p class='text-primary'>click to change configuration.</p>");
    userAgent.registrationCompvared = true;
...
function onRegisterFailed(response, cause){
    $(".reg_signal").html("Register Failed");
    if($(".reg_signal").hasClass("bg-success")){
        $(".reg_signal").removeClass("bg-success");
    }
    if($(".reg_signal").hasClass("bg-warning")){
...
function ReceiveCall(session) {
    ...
    if(buddyObj == null) {
        ...
        var autoDevare = AutoDevareDefault;
        // X-Autodevare: yes
        if(sipHeaders.hasOwnProperty("X-Autodevare")){
            if(sipHeaders["X-Autodevare"][0].raw == "yes" || sipHeaders["X-Autodevare"][0].raw == "no"){
                autoDevare = (sipHeaders["X-Autodevare"][0].raw == "yes");
                console.log("Hint Header X-Autodevare:", autoDevare)
            }
            else {
                console.warn("Hint Header X-Autodevare must yes | no :", sipHeaders["X-Autodevare"][0].raw);
            }
        }
        
        buddyObj = MakeBuddy(buddyType, true, focusOnBuddy, subscribeToBuddy, callerID, did, xmppJid, allowDuringDnd, subscribeUser, autoDevare, true);
    }
    else {
...
function AnswerAudioCall(lineNumber) {
    ...
    // Start SIP handling
    var supportedvarraints = navigator.mediaDevices.getSupportedvarraints();
    var spdOptions = {
    ...
    if(currentAudioDevice != "default"){
        ...
        if(confirmedAudioDevice) {
            spdOptions.sessionDescriptionHandlerOptions.varraints.audio.deviceId = { exact: currentAudioDevice }
        }
        else {
    ...
    // Add additional varraints
    if(supportedvarraints.autoGainControl) {
        spdOptions.sessionDescriptionHandlerOptions.varraints.audio.autoGainControl = AutoGainControl;
    }
    if(supportedvarraints.echoCancellation) {
        spdOptions.sessionDescriptionHandlerOptions.varraints.audio.echoCancellation = EchoCancellation;
    }
    if(supportedvarraints.noiseSuppression) {
        spdOptions.sessionDescriptionHandlerOptions.varraints.audio.noiseSuppression = NoiseSuppression;
    }
...
function AnswerVideoCall(lineNumber) {
    ...
    if(currentVideoDevice != "default"){
        ...
        if(confirmedVideoDevice){
            spdOptions.sessionDescriptionHandlerOptions.varraints.video.deviceId = { exact: currentVideoDevice }
        }
        else {
    ...
    // Add additional varraints
    if(supportedvarraints.frameRate && maxFrameRate != "") {
        spdOptions.sessionDescriptionHandlerOptions.varraints.video.frameRate = maxFrameRate;
    }
    if(supportedvarraints.height && videoHeight != "") {
        spdOptions.sessionDescriptionHandlerOptions.varraints.video.height = videoHeight;
    }
    if(supportedvarraints.aspectRatio && videoAspectRatio != "") {
        spdOptions.sessionDescriptionHandlerOptions.varraints.video.aspectRatio = videoAspectRatio;
    }
...
function onInviteCancel(lineObj, response){
        ...
        // Check to see if this call has been compvared elsewhere
        ...
        if (reason !== undefined && reason.length > 0){
            for (var i = 0; i < reason.length; i++){
                ...
                if (items.length >= 2 && (items[0].trim() == "sip" || items[0].trim() == "q.850") && items[1].includes("cause") && cause.includes("call compvared elsewhere")){
        ...
        if(temp_cause == 0){
            ...
            lineObj.SipSession.data.reasonText = "Call compvared elsewhere";
            console.log("Call compvared elsewhere before answer");
        }
...
async function onInviteRejected(lineObj, response){
    alert("invite rejected!");

    // $('#callProgModal').modal('show');
    // $('#callProgModalContent').html("<div class='text-center'><b>No Response.</b></div>");
    // await sleep(1000);
    // $('#callProgModal').modal('hide');
...
async function onSessionReceivedBye(lineObj, response){
    // They Ended the call
    $("#line-" + lineObj.LineNumber + "-msg").html(lang.call_ended);
    console.log("Call ended, bye!");
    // $("#callProgModalContent").html("Call ended, bye!");
    await sleep(800);
    // $("#callProgModalContent").modal("close");
    lineObj.SipSession.data.terminateby = "them";
...
function teardownSession(lineObj) {
    ...
    var session = lineObj.SipSession;
    if(session.data.teardownCompvare == true) return;
    session.data.teardownCompvare = true; // Run this code only once
...
class SoundMeter {
    varructor(sessionId, lineNum) {
        ...
        this.AnalyserNode.smoothingTimevarant = 0.85;
    }
...
function DevareQosData(buddy, stream){

    if(CallQosDataIndexDb != null){
        // Loop and Devare
        // Note:  This database can only devare based on Primary Key
        // The The Primary Key is arbitrary, so you must get all the rows based
        // on a lookup, and devare from there.
        $.each(stream.DataCollection, function (i, item) {
            if (item.ItemType == "CDR" && item.SessionId && item.SessionId != "") {
                console.log("Devaring CallQosData: ", item.SessionId);
                ...
                objectStoreGet.onsuccess = function(event) {
                    if(event.target.result && event.target.result.length > 0){
                        // There sre some rows to devare
                        $.each(event.target.result, function(i, item){
                            // console.log("Devare: ", item.uID);
                            try{
                                objectStore.devare(item.uID);
                            } catch(e){
                                console.log("Call CallQosData Devare failed: ", e);
                            }
                        });
                    }
                }
...
function SubscribeAll() {
    ...
    if(VoiceMailSubscribe){
        SubscribeVoicemail(SipUsername);
    }
...
function UnSubscribeVoicemail(SipUsername){
    if(!userAgent.isRegistered()) return;
...
function VideoCall(lineObj, dialledNumber, extraHeaders) {
    ...
    if(InviteExtraHeaders && InviteExtraHeaders != "" && InviteExtraHeaders != "{}"){
        try{
            ...
            for (var [key, value] of Object.entries(inviteExtraHeaders)) {
                if(value == ""){
...
function StartTransferSession(lineNum){
    ...
    $("#line-"+ lineNum +"-btn-compvare-transfer").hide();
    $("#line-"+ lineNum +"-btn-cancel-transfer").hide();

    $("#line-"+ lineNum +"-btn-compvare-attended-transfer").hide();
    ...
    var transferOptions  = { 
        requestDelegate: {
            onAccept: function(sip){
                ...
                session.data.transfer[transferId].accept.compvare = true;
            ...
            onReject:function(sip){
                ...
                session.data.transfer[transferId].accept.compvare = false;
                session.data.transfer[transferId].accept.disposition = sip.message.reasonPhrase;
                session.data.transfer[transferId].accept.eventTime = utcDateNow();
...
function AttendedTransfer(lineNum){
    ...
    if(session.data.AudioSourceDevice != "default"){
        spdOptions.sessionDescriptionHandlerOptions.varraints.audio.deviceId = { exact: session.data.AudioSourceDevice }
    }
    // Add additional varraints
    if(supportedvarraints.autoGainControl) {
        spdOptions.sessionDescriptionHandlerOptions.varraints.audio.autoGainControl = AutoGainControl;
    ...
    if(session.data.withvideo){
        spdOptions.sessionDescriptionHandlerOptions.varraints.video = true;
        if(session.data.VideoSourceDevice != "default"){
            spdOptions.sessionDescriptionHandlerOptions.varraints.video.deviceId = { exact: session.data.VideoSourceDevice }
        }
        // Add additional varraints
        if(supportedvarraints.frameRate && maxFrameRate != "") {
            spdOptions.sessionDescriptionHandlerOptions.varraints.video.frameRate = maxFrameRate;
    ...
    var inviterOptions = {
        requestDelegate: {
            ...
            onAccept:function(sip){
                ...
                var CompvareTransferBtn = $("#line-"+ lineNum +"-btn-compvare-attended-transfer");
                CompvareTransferBtn.off('click');
                CompvareTransferBtn.on('click', function(){
                    var transferOptions  = { 
                        requestDelegate: {
                            onAccept: function(sip){
                                ...
                                $("#line-" + lineNum + "-msg").html(lang.attended_transfer_compvare_accepted);
                    ...
                    newCallStatus.html(lang.attended_transfer_compvare);

                    updateLineScroll(lineNum);
                });
                CompvareTransferBtn.show();
        ...
        requestDelegate: {
            ...
            onProgress:function(sip){
                ...
                CancelConferenceDialBtn.on('click', function(){
                    ...
                    session.data.confcalls[confCallId].accept.compvare = false;
            ...
            onAccept:function(sip){
                newCallStatus.html(lang.call_in_progress);
                $("#line-"+ lineNum +"-btn-cancel-conference-dial").hide();
                session.data.confcalls[confCallId].compvare = true;
                ...
                JoinCallBtn.on('click', function(){
                    ...
                    session.data.confcalls[confCallId].accept.compvare = true;
...
function switchVideoSource(lineNum, srcId){
    ...
    if(srcId != "default"){
        varraints.video.deviceId = { exact: srcId }
    }

    // Add additional varraints
    if(supportedvarraints.frameRate && maxFrameRate != "") {
        varraints.video.frameRate = maxFrameRate;
    }
    if(supportedvarraints.height && videoHeight != "") {
        varraints.video.height = videoHeight;
    }
    if(supportedvarraints.aspectRatio && videoAspectRatio != "") {
        varraints.video.aspectRatio = videoAspectRatio;
    }
    ...
    navigator.mediaDevices.getUserMedia(varraints).then(function(newStream){
        ...
        console.error("Error on getUserMedia", e, varraints);
    });
...
function ShareScreen(lineNum){
    ...
    if (navigator.getDisplayMedia) {
        // EDGE, legacy support
        var screenSharevarraints = { video: true, audio: false }
        navigator.getDisplayMedia(screenSharevarraints).then(function(newStream) {
        ...
        // New standard
        var screenSharevarraints = { video: true, audio: false }
        navigator.mediaDevices.getDisplayMedia(screenSharevarraints).then(function(newStream) {
        ...
        // Firefox, apparently
        var screenSharevarraints = { video: { mediaSource: 'screen' }, audio: false }
        navigator.mediaDevices.getUserMedia(screenSharevarraints).then(function(newStream) {
...
function ShowDtmfMenu(lineNum){
    ...
    html += "<tr><td><button value='1' class='dialButtons w-100' onclick=\"sendDTMF('"+ lineNum +"', '1')\"><div>1</div><span>&nbsp;</span></button></td>"
    html += "<td><button class='dialButtons w-100' onclick=\"sendDTMF('"+ lineNum +"', '2')\"><div>2</div><span>ABC</span></button></td>"
    html += "<td><button class='dialButtons w-100' onclick=\"sendDTMF('"+ lineNum +"', '3')\"><div>3</div><span>DEF</span></button></td></tr>";
    html += "<tr><td><button class='dialButtons w-100' onclick=\"sendDTMF('"+ lineNum +"', '4')\"><div>4</div><span>GHI</span></button></td>"
    html += "<td><button class='dialButtons w-100' onclick=\"sendDTMF('"+ lineNum +"', '5')\"><div>5</div><span>JKL</span></button></td>"
    html += "<td><button class='dialButtons w-100' onclick=\"sendDTMF('"+ lineNum +"', '6')\"><div>6</div><span>MNO</span></button></td></tr>";
    html += "<tr><td><button class='dialButtons w-100' onclick=\"sendDTMF('"+ lineNum +"', '7')\"><div>7</div><span>PQRS</span></button></td>"
    html += "<td><button class='dialButtons w-100' onclick=\"sendDTMF('"+ lineNum +"', '8')\"><div>8</div><span>TUV</span></button></td>"
    html += "<td><button class='dialButtons w-100' onclick=\"sendDTMF('"+ lineNum +"', '9')\"><div>9</div><span>WXYZ</span></button></td></tr>";
    html += "<tr><td><button class='dialButtons w-100' onclick=\"sendDTMF('"+ lineNum +"', '*')\">*</button></td>"
    html += "<td><button class='dialButtons w-100' onclick=\"sendDTMF('"+ lineNum +"', '0')\">0</button></td>"
    html += "<td><button class='dialButtons w-100' onclick=\"sendDTMF('"+ lineNum +"', '#')\">#</button></td></tr>";
    html += "</table>";
    html += "</div>";
    ...
    $("#dtmfModalContent").html(html);
    // dtmf_counter = 1;
    // }
    // else{
    //     $("#callProgModalContent").remove(html);
    // dtmf_counter = 0;
    // }
...
function ShowDial(){
    ...
    // var html = "<div style=\"text-align:right\"><button class=roundButtons onclick=\"ShowContacts()\"><i class=\"fa fa-close\"></i></button></div>"
    // html += "<div style=\"text-align:center; margin-top:15px\"><input id=dialText class=dialTextInput oninput=\"handleDialInput(this, event)\" onkeydown=\"dialOnkeydown(event, this)\" style=\"width:170px; height:32px\"><button id=dialDevareKey class=roundButtons onclick=\"KeyPress('del')\">⌫</button></div>";
...
function ShowSortAnfFilter(){
    ...
    // Secondary Options
    html += "<tr><td><div><input type=checkbox id=sort_auto_devare_at_end><label for=sort_auto_devare_at_end>"+ lang.sort_auto_devare_at_end +"</label></div></td></tr>";
    html += "<tr><td><div><input type=checkbox id=sort_auto_devare_hide><label for=sort_auto_devare_hide>"+ lang.sort_auto_devare_hide +"</label></div></td></tr>";
    ...
    $("#sort_auto_devare_at_end").prop("checked", BuddyAutoDevareAtEnd==true);
    $("#sort_auto_devare_hide").prop("checked", HideAutoDevareBuddies==true);
    ...
    $("#sort_auto_devare_at_end").change(function(){
        BuddyAutoDevareAtEnd = this.checked;
        localDB.setItem("BuddyAutoDevareAtEnd", (this.checked)? "1" : "0");
        
        if(this.checked){
            $("#sort_auto_devare_hide").prop("checked", false);
            HideAutoDevareBuddies = false;
            localDB.setItem("HideAutoDevareBuddies", "0");
        }

        UpdateBuddyList();
    });
    $("#sort_auto_devare_hide").change(function(){
        HideAutoDevareBuddies = this.checked;
        localDB.setItem("HideAutoDevareBuddies", (this.checked)? "1" : "0");

        if(this.checked){
            $("#sort_auto_devare_at_end").prop("checked", false);
            BuddyAutoDevareAtEnd = false;
            localDB.setItem("BuddyAutoDevareAtEnd", "0");    
        }
...
function DialByLine(type, buddy, numToDial, CallerID, extraHeaders){
    ...
    if(buddyObj == null) {
        ...
        buddyObj = MakeBuddy(buddyType, true, false, false, (CallerID)? CallerID : numDial, numDial, null, false, null, AutoDevareDefault, false);
    }
...
function AddLineHtml(lineObj, direction){
    ...
    html += "<span class=searchClean><input id=\"line-"+ lineObj.LineNumber +"-txt-FindTransferBuddy\" oninput=\"QuickFindBuddy(this,'"+ lineObj.LineNumber +"')\" onkeydown=\"transferOnkeydown(event, this, '"+ lineObj.LineNumber +"')\" type=text autocompvare=none style=\"width:150px;\" autocompvare=none placeholder=\""+ lang.search_or_enter_number +"\"></span>";
    html += "<br>"
    ...
    html += " <button id=\"line-"+ lineObj.LineNumber +"-btn-compvare-attended-transfer\" style=\"display:none\"><i class=\"fa fa-reply-all\" style=\"transform: rotateY(180deg)\"></i> "+ lang.compvare_transfer +"</button>";
    ...
    html += "<div style=\"margin-top:10px\">";
    html += "<span class=searchClean><input id=\"line-"+ lineObj.LineNumber +"-txt-FindConferenceBuddy\" oninput=\"QuickFindBuddy(this,'"+ lineObj.LineNumber +"')\" onkeydown=\"conferenceOnkeydown(event, this, '"+ lineObj.LineNumber +"')\" type=text autocompvare=none style=\"width:150px;\" autocompvare=none placeholder=\""+ lang.search_or_enter_number +"\"></span>";
    html += "<br>"
...
function RefreshLineActivity(lineNum){
    ...
    $.each(Transfers, function(item, transfer){
        ...
        if(transfer.accept && transfer.accept.compvare == true){
            msg += lang.the_call_was_compvared
        }
        else if(transfer.accept.disposition != "") {
            msg += lang.the_call_was_not_compvared +" ("+ transfer.accept.disposition +")"
        }
        callDetails.push({
            Message : msg,
    ...
    $.each(ConfCalls, function(item, confCall){
        ...
        if(confCall.accept && confCall.accept.compvare == true){
            msg += lang.the_call_was_compvared
        }
        else if(confCall.accept.disposition != "") {
            msg += lang.the_call_was_not_compvared +" ("+ confCall.accept.disposition +")"
        }
        callDetails.push({
            Message : msg,
...
// Buddy & Contacts
// ================
var Buddy = function(type, identity, CallerIDName, ExtNo, MobileNumber, ContactNumber1, ContactNumber2, lastActivity, desc, Email, jid, dnd, subscribe, subscription, autoDevare, pinned){
    ...
    this.AllowAutoDevare = (typeof autoDevare !== 'undefined')? autoDevare : AutoDevareDefault;
 ...
 * @param {boolean} autoDevare Option to have this buddy devare after MaxBuddyAge
 * @param {boolean} addToXmppRoster Option if the buddy type is Xmpp, can automatically add to remote roster.
**/
function MakeBuddy(type, update, focus, subscribe, callerID, did, jid, AllowDuringDnd, subscribeUser, autoDevare, addToXmppRoster){
    var json = JSON.parse(localDB.getItem(profileUserID + "-Buddies"));
    if(json == null) json = InitUserBuddies();
    ...
    if(type == "extension") {
        json.DataCollection.push({
            ...
            AutoDevare: autoDevare
        });
        buddyObj = new Buddy("extension", id, callerID, did, "", "", "", dateNow, "", "", null, AllowDuringDnd, subscribe, subscribeUser, autoDevare);
    ...
    if(type == "xmpp") {
        ...
        buddyObj = new Buddy("xmpp", id, callerID, did, "", "", "", dateNow, "", "", jid, AllowDuringDnd, subscribe, subscribeUser, autoDevare);
    ...
    if(type == "contact"){
        ...
        buddyObj = new Buddy("contact", id, callerID, "", "", did, "", dateNow, "", "", null, AllowDuringDnd, false, null, autoDevare);
    ...
    if(type == "group") {
        ...
        buddyObj = new Buddy("group", id, callerID, did, "", "", "", dateNow, "", "", null, false, false, null, autoDevare);
        AddBuddy(buddyObj, update, focus, false, true);
    }
    // Update Size: 
    json.TotalRows = json.DataCollection.length;
    ...
    if(MaxBuddyAge > 1 || MaxBuddies > 1){
        ...
        if(MaxBuddyAge > 1){
            ...
            for (var b = Buddies.length - 1; b >= 0; b--) {
                ...
                if(lastActivity.isSameOrAfter(expiredDate, "second")){
                    ...
                    // Too Old
                    if(Buddies[b].AllowAutoDevare == true){
                        console.warn("This buddy is too old, and will be devared: ", lastActivity.format(DisplayDateFormat+" "+DisplayTimeFormat));
        ...
        if(MaxBuddies > 1 && MaxBuddies < Buddies.length){
            ...
            for (var b = Buddies.length - 1; b >= MaxBuddies; b--) {
                if(Buddies[b].AllowAutoDevare == true){
                    console.warn("This buddy is too Many, and will be devared: ", Buddies[b].identity);
                    DoRemoveBuddy(Buddies[b].identity);
                }
            }
        }
...
function PopulateBuddyList() {
    ...
    $.each(json.DataCollection, function (i, item) {
        item.AutoDevare = (item.AutoDevare == true)? true : false;
...
function UpdateBuddyList(){
    ...
    for(var b = 0; b < Buddies.length; b++) {
        ...
        if(HideAutoDevareBuddies){
            if(buddyObj.AllowAutoDevare) {
                hiddenBuddies++;
                continue;
            }
        }
        ...
        if(buddyObj.type == "extension") { 
            ...
            var autDevareStatus = "";
            if(buddyObj.AllowAutoDevare == true) autDevareStatus = "<i class=\"fa fa-clock-o\"></i> ";
            ...
            html += "</div>";
            html += "<div id=\"contact-"+ buddyObj.identity +"-datetime\" class=contactDate>"+ autDevareStatus + ""+ displayDateTime +"</div>";
    ...
    if(hiddenBuddies > 0){
        console.warn("Auto Devare Buddies not shown", hiddenBuddies);
...
function AddBuddyMessageStream(buddyObj) {
    ...
    profileRow += "<span class=searchClean style=\"width:100%\"><input type=text style=\"width: calc(100% - 40px);\" autocompvare=none oninput=SearchStream(this,'"+ buddyObj.identity +"') placeholder=\""+ lang.find_something_in_the_message_stream +"\"></span>";
    profileRow += "</div>";
    ...
    // ----------------------------------------------------------
    var textRow = ""
    if((buddyObj.type == "extension" || buddyObj.type == "xmpp" || buddyObj.type == "group") && EnabvarextMessaging) {
...
function RemoveBuddyMessageStream(buddyObj, days){
    ...
    if(days && days > 0){
        if(stream && stream.DataCollection && stream.DataCollection.length >= 1){
            ...
            // Create Devare Stream
            var devareStream = {
                TotalRows : 0,
                DataCollection : []
            }
            devareStream.DataCollection = stream.DataCollection.filter(function(item){
                // Apply Date Filter
                var itemDate = moment.utc(item.ItemDate.replace(" UTC", ""));
                var expiredDate = moment().utc().subtract(days, 'days');
                // Condition
            ...
            devareStream.TotalRows = devareStream.DataCollection.length;

            // Re-assign stream so that the normal devare action can apply
            stream = devareStream;
    ...
    if(stream && stream.DataCollection && stream.DataCollection.length >= 1){
        DevareCallRecordings(buddyObj.identity, stream);
    }
    
    // Remove QOS Data
    if(stream && stream.DataCollection && stream.DataCollection.length >= 1){
        DevareQosData(buddyObj.identity, stream);
    }
}
function DevareCallRecordings(buddy, stream){
    if(CallRecordingsIndexDb != null){
        // Loop and Devare
        // Note: This database can only devare based on Primary Key
        ...
        $.each(stream.DataCollection, function (i, item) {
            if (item.ItemType == "CDR" && item.Recordings && item.Recordings.length) {
                $.each(item.Recordings, function (i, recording) {
                    console.log("Devaring Call Recording: ", recording.uID);
                    var objectStore = CallRecordingsIndexDb.transaction(["Recordings"], "readwrite").objectStore("Recordings");
                    try{
                        var devareRequest = objectStore.devare(recording.uID);
                        devareRequest.onsuccess = function(event) {
                            console.log("Call Recording Devared: ", recording.uID);
                        }
                    } catch(e){
                        console.log("Call Recording Devare failed: ", e);
                    }
                });
            }
        });
...
function SortBuddies(){
    ...
    // Sort Auto Devare
    if(BuddyAutoDevareAtEnd == true){
        Buddies.sort(function(a, b){
            return (a.AllowAutoDevare === b.AllowAutoDevare)? 0 : a.AllowAutoDevare? 1 : -1;
        });
    }
    // Sort Out Pinned
    Buddies.sort(function(a, b){
    ...
    $.each(json.DataCollection, function (i, item) {
        ...
        if (item.ItemType == "MSG") {
            ...
            if(item.Displayed && item.Displayed.state == true){
                deliveryStatus = "<i class=\"fa fa-check CompvaredMessage\"></i>";
            }
...
function RedrawStage(lineNum, videoChanged){
    ...
    var i = 1;
    while (i < 5000) {
        var w = StageArea(i, videoCount, stageWidth, stageHeight, Margin, videoRatio);
        if (w === false) {
            maxWidth =  i - 1;
            break;
        }
...
function StageArea(Increment, Count, Width, Height, Margin, videoRatio) {
    ...
    var i = w = 0;
    var h = Increment * videoRatio + (Margin * 2);
    while (i < (Count)) {
...
function ShowMessageMenu(obj, typeStr, cdrId, buddy) {
    ...
    if (typeStr == "CDR") {
        ...
        // items.push({ value: 20, icon: null, text: "Devare CDR" });
                    ...
                    if(currentStream != null || currentStream.DataCollection != null){
                        $.each(currentStream.DataCollection, function (i, item) {
                            if (item.ItemType == "CDR" && item.CdrId == cdrId) {
                                ...
                                console.log("Poster Image Devared");
                                return false;
                            }
                        });
                        localDB.setItem(buddy + "-stream", JSON.stringify(currentStream));
...
function AddMenu(obj, buddy){
    ...
    var items = [];
    if(EnabvarextExpressions) items.push({ value: 1, icon : "fa fa-smile-o", text: lang.select_expression });
    if(EnabvarextDictate) items.push({ value: 2, icon : "fa fa-microphone", text: lang.dictate_message });
    // TODO
...
function ShowMyProfile(){
    ...
    window.setTimeout(function(){
        ...
        selectMicScr.change(function(){
            ...
            navigator.mediaDevices.getUserMedia(varraints).then(function(mediaStream){
        ...
        if(EnableVideoCalling == true){
            ...
            selectVideoScr.change(function(){
                ...
                if($("input[name=Settings_FrameRate]:checked").val() != ""){
                    varraints.video.frameRate = $("input[name=Settings_FrameRate]:checked").val();
                }
                if($("input[name=Settings_Quality]:checked").val() != ""){
                    varraints.video.height = $("input[name=Settings_Quality]:checked").val();
                }
                if($("input[name=Settings_AspectRatio]:checked").val() != ""){
                    varraints.video.aspectRatio = $("input[name=Settings_AspectRatio]:checked").val();
                } 
                console.log("varraints:", varraints);
                var localStream = new MediaStream();
            ...
            frameRateSel.change(function(){
                ...
                if(this.value != ""){
                    varraints.video.frameRate = this.value;
                }
                if($("input[name=Settings_Quality]:checked").val() != ""){
                    varraints.video.height = $("input[name=Settings_Quality]:checked").val();
                }
            ...
            QualitySel.change(function(){    
                ...
                if(this.value){
                    varraints.video.height = this.value;
                }
                if($("input[name=Settings_AspectRatio]:checked").val() != ""){
                    varraints.video.aspectRatio = $("input[name=Settings_AspectRatio]:checked").val();
                } 
            ...
            AspectRatioSel.change(function(){    
                ...
                if(this.value != ""){
                    varraints.video.aspectRatio = this.value;
                }        
                console.log("varraints:", varraints);
                var localStream = new MediaStream();
                if(navigator.mediaDevices){
...
function RefreshRegistration(){
    Unregister();
    console.log("Unregister compvare...");
    window.setTimeout(function(){
...
function CreateImageEditor(buddy, placeholderImage){
    ...
    toolBarDiv.append('<button class="toolBarButtons" title="Devare Selected Items" onclick="ImageEditor_Clear(\''+ buddy +'\')"><i class="fa fa-times"></i></button>');
